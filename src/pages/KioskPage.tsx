...
import { ScanBarcode, ArrowDownToLine, ArrowUpFromLine, CheckCircle2, XCircle } from 'lucide-react';
...
export default function KioskPage() {
  ...
  const handleScan = async () => {
    ...
    try {
      ...
      if (traineeErr || !trainee) {
        setResult({ name: 'مجهول', status: 'error', message: 'لم يتم العثور على المتدرب' });
        clearResult();
        setBarcode('');
        setScanning(false);
        return;
      ...
      if (!schedule) {
        setResult({ name: trainee.full_name, rank: trainee.rank ?? undefined, status: 'error', message: 'لا توجد نوبة محددة اليوم' });
        clearResult();
        setBarcode('');
        setScanning(false);
        return;
      ...
      if (mode === 'IN') {
        ...
        const scheduledMinutes = Math.floor((shiftEnd.getTime() - shiftStart.getTime()) / 60000);

        const { error: insertErr } = await supabase.from('attendance_sessions').upsert({
        ...
        setResult({
          ...
          status: 'success',
          message: isLate ? `تم تسجيل الدخول — متأخر (${lateMinutes} دقيقة)` : 'تم تسجيل الدخول — في الوقت المحدد',
        });
      } else {
        // Check-out
        ...
        if (!session || !session.check_in_at) {
          setResult({ name: trainee.full_name, rank: trainee.rank ?? undefined, status: 'error', message: 'لم يتم العثور على تسجيل دخول لاليوم' });
          clearResult();
          setBarcode('');
          setScanning(false);
          return;
        ...
        const actualMinutes = Math.floor((now.getTime() - checkIn.getTime()) / 60000);
        const lostMinutes = Math.max(0, session.scheduled_minutes - actualMinutes);
        ...
        setResult({
          ...
          rank: trainee.rank ?? undefined,
          status: 'success',
          message: earlyLeave > 0 ? `تم تسجيل الخروج — مبكر (${earlyLeave} دقيقة)` : 'تم تسجيل الخروج — مكتمل',
        });
      }
    } catch (err: any) {
      setResult({ name: '', status: 'error', message: err.message ?? 'فشل المسح' });
    }
  ...
  return (
    <div className="flex min-h-[calc(100vh-3.5rem)] flex-col items-center justify-center bg-kiosk-bg p-8 -m-6">
      ...
      <Card className="w-full max-w-lg border-2 border-sidebar-border bg-kiosk-card shadow-2xl">
        <CardContent className="p-8">
          <div className="mb-6 flex items-center justify-center gap-3">
            ...
            <h2 className="text-2xl font-bold text-accent">مسح الرمز الشريطي</h2>
          </div>
          <form
            onSubmit={(e) => {
              e.preventDefault();
      ...
      {/* Result */}
      {result && (
        <div className={`mt-8 w-full max-w-lg animate-slide-in rounded-lg border-2 p-6 text-center ${
          result.status === 'success' ? 'border-success bg-success/10' : 'border-destructive bg-destructive/10'
        }`}>
          ...
          <p className={`mt-1 text-lg font-medium ${result.status === 'success' ? 'text-success' : 'text-destructive'}`}>
            {result.message}
          </p>
        </div>
      )}
