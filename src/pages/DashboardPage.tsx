...
export default function DashboardPage() {
  ...
  const { data: traineesCount } = useQuery({
    ...
    queryFn: async () => {
      const { count } = await supabase.from('trainees').select('*', { count: 'exact', head: true }).eq('status', 'active');
      return count ?? 0;
    },
  });
  ...
  const { data: attendance } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from('attendance_sessions').select('status').eq('day_date', date);
      const sessions = data ?? [];
      return {
  ...
  const stats = [
    { title: 'إجمالي المتدربين', value: traineesCount ?? 0, icon: Users, color: 'text-primary' },
    { title: 'حاضرون', value: (attendance?.present ?? 0) + (attendance?.late ?? 0), icon: UserCheck, color: 'text-success' },
    { title: 'غائبون', value: attendance?.absent ?? 0, icon: UserX, color: 'text-destructive' },
    { title: 'هاربون', value: attendance?.escaped ?? 0, icon: AlertTriangle, color: 'text-warning' },
  ];
    ...
    <div className="space-y-6 animate-slide-in">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">نظرة عامة على الحضور والإحصائيات</p>
        </div>
      ...
      <Card>
        ...
        <CardContent>
          {recentEscapes && recentEscapes.length > 0 ? (
              ...
              {recentEscapes.map((e: any) => (
                <div key={e.id} className="flex items-center justify-between rounded-md border p-3">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">{e.trainees?.rank} • {e.type}</p>
                  </div>
            ...
            <p className="text-sm text-muted-foreground">لا توجد أحداث هروب في هذا التاريخ.</p>
          )}
        </CardContent>
      </Card>
    </div>
