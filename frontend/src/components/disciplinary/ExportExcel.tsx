...
export function ExportExcel({ data }: ExportExcelProps) {
  const generateExcel = async () => {
    ...
    const worksheet = workbook.addWorksheet(i18n.t("disciplinary.title"));
    ...
    const headers = [
      "",
      i18n.t("disciplinary.requestDate"),
      i18n.t("common.name"),
      i18n.t("disciplinary.reasonCol"),
    ...
    // ── Data rows ─────────────────────────────────────────────────────────
    data.forEach((d: DisciplinaryRequest, i: number) => {
      const isEven = i % 2 === 0;
    ...
    a.href = url;
    a.download = `${i18n.t("disciplinary.filename", { date: getTodayDateKSA() })}.xlsx`;
    a.click();
